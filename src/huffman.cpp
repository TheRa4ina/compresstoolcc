...
CharBitMap Huffman::buildDictionary(std::istream& is)
{
...
std::unordered_map < std::string , freq_t > Huffman::getFrequency(std::istream& is)
{
...
Node<Frequency> Huffman::buildTree(std::unordered_map<std::string, freq_t> char_frequency)
{
...
CharBitMap Huffman::helperBuildDictionary(std::shared_ptr<const Node<Frequency>> cur_node, std::bitset<BITSET_SIZE> cur_bits) const
{
	if (cur_node->isLeaf()) {
